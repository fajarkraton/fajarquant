...
| F.6.4 | Hadamard at Base or Medium scale (rather than Mini) | ✅ **CLOSED 2026-05-02** with **NULL @ Medium** verdict via `docs/FJQ_PHASE_F_F6_4_DECISION.md`. Three full ablation cells (Base baseline, Medium baseline, Medium hadamard) ran 13.8h GPU on RTX 4090. **Δ_medium = +0.025 nat** falls in the (−0.05, +0.05) NULL band — neither clear win nor clear loss. Cross-scale: regression gap **narrows 4.7×** from Mini (Δ=+0.120) to Medium (Δ=+0.025). Two hypotheses preserved without commitment: SpinQuant-aligned scale-dependent benefit OR convergence-noise-dominates. Δ_base unmeasurable (d=384 ≠ 2^k structurally). Paper v2 §7.5 update gated on founder Edit A/B/C decision. |
