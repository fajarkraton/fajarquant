# Phase E E2 — Algorithm Catch-up Pre-flight Audit (Findings v1.1)

> **Status:** E2.0 CLOSED — Q1–Q5 all closed; go/no-go decision = **GO** for E2.1 (Hadamard rotation) per §5.1 below. Audit + plan + Q5 empirical baseline complete per CLAUDE §6.8 R1.
>
...
>
> **Last updated:** 2026-04-27 (Phase E v1.8, **v1.1 promotion** — E2.0 CLOSED, all 5 blockers down, Q5 baseline measured at Mini scale via `scripts/q5_bilingual_baseline.py`).
...
| ~~Q5~~ ✅ **CLOSED v1.1** | What's the bilingual coherence baseline (ratio val_loss(ID)/val_loss(EN)) for a Mini-scale bilingual training? | E2.5.4 gate is "ratio ≤1.5× at Mini scale". Need to know the no-feature baseline ratio to interpret deltas. | **Answer:** Mini-scale (22M params, 6 layers, hidden 256) trained 24,000 steps on `bilingual_stream(id_share=0.6)` (= 200M tokens at 60:40 ID:EN) via new `scripts/q5_bilingual_baseline.py`. Wall: 42 min on RTX 4090 Laptop. Held-out val (50 batches each side, distinct seeds): **val_loss(ID) = 2.6786 nat (PPL 14.6)**, **val_loss(EN) = 4.7317 nat (PPL 113.5)**, **ratio = 1.7665× (EN is the worse side)**. Initial loss 10.43 (≈ ln(32768) random-init floor); final train-step loss 3.05. Artifact: `paper/intllm/ablations/q5_bilingual_baseline.json` (schema v1.0, separate from E2.x ablation rows). **Implication for E2.5.4 gate:** the no-feature baseline already FAILS the ratio ≤1.5× gate at 1.77× — i.e. naive 60:40 bilingual training has a coherence gap of ~0.27× above the gate. This is *informative*, not blocking: it sets the headroom that E2.5 (lang-conditioned design) must close. Two readings consistent with the data — (a) the gate threshold itself may need recalibration if 1.77× is the genuine "no-feature floor" at Mini scale (decision deferred to E2.5.4 doc), or (b) E2.5 Option (a) per-lang RMSNorm γ delivers the closure (preliminary recommendation per Q2 closure stays — testable at E2.5.3). **Side observation:** ID side fits dramatically better than EN at this scale despite 60:40 mix (PPL 14.6 vs 113.5 = 7.8× PPL ratio). Plausible drivers: (1) Indonesian Wikipedia + FineWeb-2 has higher template-redundancy than SlimPajama EN (Wikipedia stub-pages, FineWeb-2 admin pages); (2) Mistral v3 BPE is English-trained → ID texts hit lower-entropy fallback chars more often (lower per-token surprise); (3) 80M EN tokens is below the Mini saturation point (Phase D Mini v2 at 491M EN reached val_loss 4.38). EN side at 80M tok / val_loss 4.73 is consistent with a half-trained Phase D Mini, NOT an architectural failure mode. |
...
This findings doc is **v1.1 (E2.0 CLOSED, 2026-04-27)**. All five ship criteria are met:
...
- [x] **Q5 bilingual coherence baseline measured** (closed in §4 above): val_loss(ID) = 2.68, val_loss(EN) = 4.73, **ratio = 1.77×** (EN worse). 24K steps × 8K tok/step = ~196M tokens at 60:40 on Mini-scale HGRNBit. Wall: 42 min RTX 4090. Artifact: `paper/intllm/ablations/q5_bilingual_baseline.json`.
- [x] One paragraph "go/no-go" decision per CLAUDE §6.8 R6 — see §5.1 below (decision: **GO** for E2.1 with E2.5 priority elevated).

**Actual effort to v1.1:** ~6h human (this session: data plumbing + driver + smoke + writeup) + ~42 min GPU (Q5 full run; well under the 3-5h estimate because Mini-scale bilingual streams equal Phase D Mini in throughput). Variance: −86% on GPU side (42 min vs 3-5h), +0% on human side. Source of variance: Q5 estimate budgeted parquet-read overhead that did not materialize — local pyarrow shard reads kept up with GPU at 95% utilization without buffering the data loader.

### 5.1 Go/no-go decision (per CLAUDE §6.8 R6)

**Decision: GO for E2.1 (Hadamard rotation), with E2.5 (language-conditioned design) priority elevated above its plan-v1.8 ordering.**

Rationale:
1. **All 5 blocker questions closed.** E2.0 ship criteria fully met; nothing in Q1–Q5 contradicts the plan-v1.8 §3 sub-task ordering or surface-area assumptions.
2. **Q5 ratio = 1.77× is informative, not blocking.** The E2.5.4 ratio ≤1.5× gate fails at the no-feature baseline by ~0.27×. This means: (a) E2.1+E2.2+E2.3+E2.4 ablations (which do NOT directly target language coherence) should be benchmarked against this baseline ratio, not against 1.5× directly — a feature is "neutral on coherence" if ratio stays in 1.7×–1.8×, not if it stays under 1.5×. (b) E2.5 carries the explicit responsibility to close the ~0.27× gap, and its option-selection (a/b/c) decision must measure ratio-improvement relative to 1.77×, not relative to 1.5×.
3. **No surprise data.** EN side at 80M tokens reaching val_loss 4.73 is consistent with Phase D Mini v2 trajectory at the equivalent token count (a half-trained EN Mini, not an architectural failure). ID side outperforming EN by 7.8× in PPL despite 60:40 mix is consistent with corpus-redundancy + tokenizer-fit factors (see Q5 closure side observation), not with bilingual interference. Therefore the standard plan-v1.8 ordering remains valid.
4. **Mechanical gate satisfaction.** This decision file (`FJQ_PHASE_E_E2_FINDINGS.md` v1.1 sign-off below) is the committed-file artifact required by §6.8 R6. Downstream (E2.1.x sub-tasks) can reference this section by anchor; pre-commit hooks gate via the `make verify-bilingual-corpus` invariants (which only assert corpus consistency, not E2.0 closure — that's by-design, since E2.0 closure is a one-shot promotion event, not a recurring check).

Adjustment to plan-v1.8 §3 sequencing: E2.5 sub-task gating now requires its delta-ratio improvement to be measured *relative to the Q5 baseline ratio of 1.77×*, not relative to a flat 1.5×. The E2.5.4 decision doc, when written, should explicitly state which threshold it adopts and why. This is a refinement of the gate, not a retreat: a feature that drops ratio from 1.77× to 1.55× should still be considered a clear partial win even if it does not cross the absolute 1.5× line.
...
- Q5 driver: `python/phase_d/scripts/q5_bilingual_baseline.py` (this session)
- Q5 data plumbing: `python/phase_d/intllm/data.py::indonesian_parquet_stream`, `::bilingual_stream` (this session, 8 unit tests in `tests/test_data.py`)
- Q5 result: `paper/intllm/ablations/q5_bilingual_baseline.json` (schema v1.0)
...
**Sign-off (v1.1, 2026-04-27):** Phase E2.0 pre-flight CLOSED. All 5 blocker questions Q1–Q5 closed; Q5 empirical baseline measured at Mini scale (24K steps × 8K tok/step ≈ 196M tokens at 60:40 ID:EN, 42 min RTX 4090). Coherence ratio at no-feature baseline = 1.77× (EN worse), 0.27× above the E2.5.4 absolute gate; informative for E2.5 sub-task gating, not blocking for E2.1. **Decision: GO for E2.1 (Hadamard rotation) with E2.5 priority elevated** (see §5.1). Plan-v1.8 §3 PHASE E2 sub-task ordering remains valid; only E2.5.4's threshold framing is refined to delta-ratio against 1.77× baseline.

Next session natural first step: E2.1.1 — implement `intllm.quant.HadamardRotation` per QuaRot/SpinQuant recipe, applied to `block.attn.o_proj` only (per Q1 closure). Smoke via `make train-mini-ablation TAG=hadamard --hadamard --proof-of-life` (E2.1.0 scaffold landed in commit `5ba7858`).

*Document version: 1.1 (E2.0 CLOSED). Author: Claude Opus 4.7 + Fajar (PrimeCore.id).*
