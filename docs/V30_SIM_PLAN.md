...
| P2.3 ✅ | Kernel `FJTRACE=1` mode: add env-flag-gated `println` at same op boundaries | `make build-fjtrace` compiles (FJTRACE=1 path); 17 emit sites at same boundaries as `OP_NAMES`; FNV-1a constants byte-match; ELF delta = +5312 bytes trace code | ~0.7h (+40%) — SE001 on forward-ref, re-routed via drivers/serial.fj |
| P3.2.I ✅ | **Capture infrastructure** (new subphase) — `make test-fjtrace-capture` in fajaros-x86 automates: build-fjtrace + QEMU boot with NVMe + auto-feed `ask hello` + parse → clean JSONL | Makefile target parses dry-run; params `FJTRACE_DISK`, `FJTRACE_TIMEOUT`, `FJTRACE_PROMPT`, `FJTRACE_BOOT_WAIT`, `FJTRACE_RUN_WAIT` configurable; outputs `build/fjtrace-capture.{log,jsonl,stats}`; exits 2 if <17 records (false-start detection) | ~0.2h (new, not in original plan — P3 plan assumed `run-nvme-llvm` which doesn't exist) |
| P2.4 ✅ | `parse_kernel_trace.py`: serial log → same JSONL schema | `--self-test` 7/7 PASS against bundled fixture (17 valid records + 2 malformed + 1 unknown-op); CLI supports stdin/stdout, --strict, --renumber; schema validator catches schema-version mismatch, missing fields, unknown ops | ~0.3h (+50%) — self-test fixture + round-trip assertion |
| P2.5 ✅ | HF reference forward on same prompt, dumped as float baseline | `--self-test` 7/7 PASS (91 records for N=3/L=2 synth config, all 17 ops hit, round-trip through parse_kernel_trace.py clean); `--dry-run -o /tmp/hf_trace.jsonl --prompt hello` emits 151 records (5-token × (1+2×14+1)+1) with real top5_abs from HF float | ~0.6h (+100%) — 3 op-boundary ops needed MLP+DecoderLayer monkey-patch |
