#!/usr/bin/env python3
"""
V32-prep F.6.4 — Base-scale ablation harness.
...
`paper/intllm/ablations/base_<TAG>.json` artifacts so any feature
demoted from Mini-scale Phase E2 to F.6.4-scale re-evaluation can be
re-run at Base scale (d=384, L=12, 73 BitLinear sites = 6×12 attn/MLP
...
toggles. Mirrors `train_base.py` (Phase D) closely so behavior is
...
Full (~4 h on RTX 4090 per CONFIG matrix §3.2, baseline equivalent to Phase D Base c.1):
...
JSON schema written to `paper/intllm/ablations/base_<TAG>.json`:
...
# Base gate per FJQ_PHASE_D_GATE_CALIBRATION.md (calibrated 2026-04-21).
BASE_VAL_LOSS_GATE = 4.2
...
#
# `BaseTrainConfig.n_steps = 60000` is the original Phase D Base-full
# target (~491M tokens, EN-only); use that only by passing --n-steps
# 60000 explicitly when reproducing Phase D Base gates, not when
...
def _write_ablation_json(
    ...
    *,
    ...
    payload = {
        ...
        "gate_pass": (val_loss < BASE_VAL_LOSS_GATE) if not val_skipped else None,
        "gate_threshold": BASE_VAL_LOSS_GATE,
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Phase E E2 Base-scale ablation harness (E2.1.0 scaffold).",
    )
    p.add_argument(
        "--tag",
        required=True,
        help="Ablation tag — output goes to paper/intllm/ablations/base_<TAG>.json. "
    ...
    p.add_argument("--ckpt-dir", type=Path, default=HERE.parent / "checkpoints" / "base_ablations")
    p.add_argument("--device", default=None)
    p.add_argument(
        "--out",
        type=Path,
        default=None,
        help="Override output JSON path. Default: paper/intllm/ablations/base_<TAG>.json",
    )
    ...
    # F.10.4 — Sparse-BitNet 2:4 structured sparsity flag (Base scale)
    ...
    p.add_argument(
        ...
        "Default OFF (= slimpajama EN-only, matches Phase D Base gates). "
    ...
    if args.dry_run:
        ...
        from configs.base import BaseArchConfig
    ...
    if args.proof_of_life:
        ...
        # NOT BaseTrainConfig's 60K. See E2_DEFAULT_N_STEPS docstring.
    ...
    if use_bilingual:
        ...
        print("      stream: slimpajama (EN-only, Phase D Base-gate baseline)")
    ...
    #
    # Single shared HadamardRotation instance — all o_proj sites at Base
    # share `hidden_size` (=384), so one matrix in memory suffices. Hook
    ...
    # is set. Must happen BEFORE optimizer construction (parameters change
    # identity after replacement). See train_mini_ablation.py for the
    # canonical version of this wiring block.
    if args.sparse_2_4:
    ...
    if trackers is not None:
        ...
        # so for L=12 (Base) we expect 6×12 + 1 = 73. Print the layer
        # count for smoke confirmation.
        tracker_maps_path = ABLATIONS_DIR / f"base_{args.tag}_maps.pt"
        save_calibration_maps(
            trackers,
            tracker_maps_path,
    ...
    if args.balanced_calib and not args.proof_of_life and tracker_maps_path is not None:
        ...
        quant_error_path = ABLATIONS_DIR / f"base_{args.tag}_quant_error.json"
        qe_t0 = time.time()
    ...
    if not args.proof_of_life:
        ...
        print(
            f"  Base gate     : val_loss < {BASE_VAL_LOSS_GATE} -> "
            f"{'PASS ✓' if val_loss < BASE_VAL_LOSS_GATE else 'FAIL ✗'}"
        )
    ...
    return 0 if val_loss < BASE_VAL_LOSS_GATE else 1
