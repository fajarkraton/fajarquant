...
def main() -> int:
    ...
    resume_grp.add_argument("--resume-auto", action="store_true",
                            help="Resume from the highest-step checkpoint found in --ckpt-dir")
    p.add_argument("--watchdog-idle-seconds", type=int, default=1800,
                   help="SIGTERM if step counter idle > N seconds. 0=disabled. "
                        "Default 1800 (30 min) — covers real pauses, catches hangs.")
    args = p.parse_args()
    ...
    # Track B step 1 (V31.C.P6.1): wire train_hp.ckpt_every so interrupted
    # runs can resume. POL skips (too-few steps to matter).
    ckpt_every = 0 if args.proof_of_life else train_hp.ckpt_every

    # Track B step 2 (V31.C.P6.2): resolve --resume / --resume-auto into
    # a concrete path, or None if starting fresh.
