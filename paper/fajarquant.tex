...
PCA-rotation advantage is architecture-agnostic. Mistral data is from
50 prompts ($n{=}12{,}800$ samples per cell); Qwen2-7B is from 5 prompts
($n{=}560$). Gemma~4~E2B numbers are from Table~\ref{tab:ablation}
($n{=}1{,}500$). The KV-head ratio column reports attention\_heads :
KV\_heads, where $1{:}1$ is full multi-head attention.}
...
Qwen2-7B     & GQA      & 28 & $28{:}4$ & $\mathbf{7.70}$ & $\mathbf{+5.1\%}$ & $\mathbf{+5.6\%}$ & $\mathbf{+6.1\%}$ \\
\bottomrule
\end{tabular}
\end{table*}
...
entirely to $+0.2\%$ (statistical tie), versus $-2.8\%$ on Gemma and
$-11.1\%$ on Mistral at the same bit width. This is the first model
in our evaluation where FajarQuant's value performance matches
TurboQuant's at the most aggressive quantization level.
